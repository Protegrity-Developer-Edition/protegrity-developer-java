Feature: Successful Protection Operations For Bytes Input Data
    ...
    Scenario: Tokenize and Detokenize bulk bytes input data
        When the user performs "tokenization" operations on bulk bytes input data using the following data elements
            ...
            | address      | 123 Main St Springfield,456 Elm St Shelbyville,789 Oak Ave Capital City,101 Maple Rd Smalltown                                                                                                                    |
            | city         | Springfield,Shelbyville,Capital City,Smalltown                                                                                                                                                                    |
            | postcode     | SW1A 1AA,M1 1AA,B1 1AA,L1 1AA                                                                                                                                                                                     |
            | zipcode      | 90210,10001,60601,94105                                                                                                                                                                                           |
            | phone        | +1-202-555-0173,+44-20-7946-0958,+49-30-123456,+33-1-23456789                                                                                                                                                     |
            | email        | johndoe@example.com,janesmith@domain.co.uk,alicej@web.de,boblee@free.fr                                                                                                                                           |
            | datetime     | 2023-09-04T14:30:00,2023-10-15T09:15:30,2023-11-20T16:45:00,2023-12-25T12:00:00                                                                                                                                   |
            | int          | 12345,67890,23456,78901                                                                                                                                                                                           |
            | nin          | QQ123456C,AB987654D,XY456789E,ZZ111222F                                                                                                                                                                           |
            | ssn          | 123-45-6789,987-65-4321,111-22-3333,444-55-6666                                                                                                                                                                   |
            | ccn          | 4111111111111111,5500000000000004,340000000000009,30000000000004                                                                                                                                                  |
            | ccn_bin      | 4111111122223333,5500000022223333,340000002222333,30000000222233                                                                                                                                                  |
            | passport     | X1234567,Y7654321,Z9876543,A1122334                                                                                                                                                                               |
            | iban         | GB82WEST12345698765432,DE89370400440532013000,FR1420041010050500013M02606,ES9121000418450200051332                                                                                                                |
            | iban_cc      | 12345678901234567890,09876543210987654321,11223344556677889900,22334455667788990011                                                                                                                               |
            | string       | The 'string' data element protects all alphabetic symbols (both lowercase and uppercase letters) as well as digits 1234.,Uppercase and lowercase letters.,Special characters are ignored.,Bulk input test string. |
            | number       | 9876543210 11 234,1122 33445 5,2233445566,3344556677                                                                                                                                                              |
            | name_de      | Max Mustermann,Anna Schmidt,Peter Müller,Julia Fischer                                                                                                                                                            |
            | name_fr      | Jean Dupont,Marie Curie,Émile Zola,Lucie Bernard                                                                                                                                                                  |
            | address_de   | Musterstraße 12 München,Bahnhofstr. 5 Berlin,Hauptstr. 7 Hamburg,Marktplatz 1 Köln                                                                                                                                |
            | address_fr   | 10 Rue de Rivoli Paris,20 Avenue Victor Hugo Lyon,5 Boulevard Saint-Michel Marseille,15 Rue de la Païx Nice                                                                                                       |
            | city_de      | München,Berlin,Hamburg,Köln                                                                                                                                                                                       |
            | city_fr      | Paris,Lyon,Marseille,Nice                                                                                                                                                                                         |
        Then all the bulk bytes data is validated
        And all the protected data should not equal the original data
        And all the unprotected data should equal the original data

    @encryption @single @bytes @positive
    Scenario: Encrypt and Decrypt single bytes input data
        When the user performs "encryption" operations on single bytes input data using the following data elements
            | data_element | input_data                                                                                                                    |
    ...
    @encryption @bulk @bytes @positive
    Scenario: Encrypt and Decrypt bulk bytes input data
        When the user performs "encryption" operations on bulk bytes input data using the following data elements
            | data_element | input_data                                                                                                                    |
            | text         | The Encryption data Element, Encrypts all alphabetic symbols, (both lowercase and uppercase letters), as well as digits 5678. |
        Then all the bulk bytes data is validated
        And all the protected data should not equal the original data
    ...
    @tokenization @single @eiv @bytes @positive
    Scenario: Tokenize and Detokenize single bytes input data with External Initialization Vector (EIV)
        When the user performs "tokenization" operations on single bytes input data with external IV using the following data elements
        ...
        And the same external IV produces consistent protected values for bytes
        And all the unprotected data should equal the original data

    @tokenization @bulk @eiv @bytes @positive
    Scenario: Tokenize and Detokenize bulk bytes input data with External Initialization Vector (EIV)
        When the user performs "tokenization" operations on bulk bytes input data with external IV using the following data elements
            | data_element | input_data                                                                                                                                                                                                                                                                                               | external_iv       |
