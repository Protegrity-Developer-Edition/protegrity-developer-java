Feature: Successful Protection Operations For String Input Data
    ...
    Scenario: Tokenize and Detokenize single string input data
        When the user performs "tokenization" operations on single string input data using the following data elements
            ...
            | city_de                 | München                                                                                                                   |
            | city_fr                 | Paris                                                                                                                     |
            | fpe_numeric             | 11212 232324                                                                                                              |
            | fpe_alpha               | Max Mustermann                                                                                                            |
            | fpe_alphanumeric        | Max Mustermann 1213232                                                                                                    |
            | fpe_latin1_alpha        | Océaneabcd123456Inèsprotegrity                                                                                            |
            | fpe_latin1_alphanumeric | abcdefg13245àça527174Ç3525945387139985Ò8786ÕÖØ52äæ                                                                        |
    ...
    Scenario Outline: Encyrpt and Decrypt bulk string input data
        When the user performs "encryption" operations on bulk string input data using the following data elements
            | data_element | input_data                                                                                                                    |
            | text         | The Encryption data Element, Encrypts all alphabetic symbols, (both lowercase and uppercase letters), as well as digits 5678. |
    ...
    Scenario: Tokenize and Detokenize bulk string input data
        When the user performs "tokenization" operations on bulk string input data using the following data elements
            ...
            | fpe_numeric             | 9876543210 11 234, 1122 33445 5, 2233445566, 3344556677, 4455667788, 5566778899, 6677889900, 7788990011                                                                                                                |
            | fpe_alpha               | Springfield, Shelbyville, Capital City, Smalltown, Bigcity, Midtown, Uptown, München                                                                                                                                   |
            | fpe_alphanumeric        | 10 Rue de Rivoli, Paris, 20 Avenue Victor Hugo, Lyon, 5 Boulevard Saint-Michel, Marseille, 15 Rue de la Païx, Nice, 8 Place Bellecour, Toulouse, 12 Rue du Bac, Bordeaux, 3 Avenue Foch, Lille, 7 Rue Gambetta, Nantes |
            | fpe_latin1_alpha        | Océaneabcd123456Inèsprotegrity, ÆÈðñabcdfsfd1234protegrity, abcdefg13245àça527174Ç3525945387139985Ò8786ÕÖØ52äæ                                                                                                         |
            | fpe_latin1_alphanumeric | Océaneabcd123456Inèsprotegrity, ÆÈðñabcdfsfd1234protegrity, abcdefg13245àça527174Ç3525945387139985Ò8786ÕÖØ52äæ                                                                                                         |
        Then all the bulk data is validated
        And all the protected data should not equal the original data
        And all the unprotected data should equal the original data
    ...
    Scenario: Tokenize and Detokenize bulk string input data with External Initialization Vector (EIV)
        When the user performs "tokenization" operations on bulk string input data with external IV using the following data elements
            | data_element | input_data                                                                                                                                                                                                                                                                                               | external_iv       |
    ...
    @masking @single @string @positive
    Scenario: Mask single string input data - unprotect returns masked value for user without unprotect permission
        Given the current user is set to "hr"
        When the user performs "tokenization" unprotect operation on single string input data using the following data elements
            | data_element | input_data          |
            | mask         | John Doe            |
            | mask         | Jane Smith          |
            | mask         | Alice Johnson       |
            | mask         | 123-45-6789         |
            | mask         | johndoe@example.com |
        Then the unprotect output is entirely masked with "*" character
